...
cfg = cfg.getconfig()
#query_url = https://data.exactspace.co/exactapi
...
UNIT_ID = "61c1818371c20d4a206a2e35" #jklc3
#UNIT_ID = "61c0c34bb45a623b64fc3b12" #bhrgblr
#UNIT_ID = "630c916f8c19444ca8aed6fb"  # vietnam
#UNIT_ID = "61caeda654bf6a5bf94bf59a"
#UNIT_ID = "62e9106d75c9b4657aebc8fb"
#UNIT_ID ="611b7fd57afa992d36e308b0"
...
def persistGroupByData(request):
    ...
    # url = cfg["api"]["meta"] + '/units/' + unitId + '/incidents?filter={"where":{"systemName" : "' + groupBy + '", "or":[{"and":[{"open":true},"startTime":{"gte":'+startTime[0:10]+"T00:00:00.000Z"+'}},{"and":[{"startTime":{"gte":"'+startTime[0:10]+"T00:00:00.000Z" + '"},"startTime":{"lte":"'+startTime[0:10]+"T23:59:59.000Z" + '"}}]}],"incidentType":{"inq":["operations","Operations","v2"]}},"fields":{"startTime":true,"endTime":true,"faulttreeId":true,"systemName":true, "equipmentName":true}}'
    equips = sysEqp[groupBy]
    
    #print("grrrrrrrp",groupBy)
    
    #url = cfg["api"]["meta"] + '/incidents?filter={"order":"startTime DESC","where":{"systemName" : "' + groupBy + '","unitsId":"'+ unitId +'","incidentType":{"inq":["Operations","operations","v2"]},"or":[{"and":[{"startTime":{"gte":"'+startTime[0:10]+"T00:00:00.000Z"+'"}},{"startTime":{"lte":"'+ startTime[0:10]+"T23:59:59.000Z" +'"}}]},{"and":[{"open":true},{"startTime":{"gte":"'+startTime[0:10]+"T00:00:00.000Z"+'"}}]}]}}'
    
    url = cfg["api"]["meta"] + '/incidents?filter={"order":"startTime DESC","where":{"systemName" : "' + groupBy + '","unitsId":"'+ unitId +'","incidentType":{"inq":["Operations","operations","v2","V2"]},"or":[{"and":[{"open":false},{"endTime":{"gte":"'+endTime[0:10]+"T23:59:59.000Z"+'"}},{"startTime":{"lte":"'+ endTime[0:10]+"T00:00:00.000Z" +'"}}]},{"and":[{"open":true},{"startTime":{"lte":"'+endTime[0:10]+"T00:00:00.000Z"+'"}}]}]}}'
    
    equips = sysEqp[groupBy]
    print("uuuuuuuuuu",url)
    try:
        incidents = requests.get(url).json()
        #print("hhhhhhhhooooooo",incidents)
        equipments = []
...
def persistCalendarData(request):
    ...
    lastMonth = now - timedelta(days=30)
    _startT = lastMonth.strftime("%Y-%m-%dT%H:%M:%S.000Z")
    print("startmonth",_startT,lastMonth)
    _endT = now.strftime("%Y-%m-%dT%H:%M:%S.000Z")
    groups = []
    ...
    #url = cfg["api"]["meta"] + '/incidents?filter={"order":"startTime DESC","where":{"unitsId":"'+ unitId +'","incidentType":{"inq":["Operations","operations","v2"]},"or":[{"and":[{"startTime":{"gte":"'+startTime[0:10]+"T00:00:00.000Z"+'"}},{"startTime":{"lte":"'+ startTime[0:10]+"T23:59:59.000Z" +'"}}]},{"and":[{"open":true},{"startTime":{"gte":"'+startTime[0:10]+"T00:00:00.000Z"+'"}}]}]}}'
    
    url = cfg["api"]["meta"] + '/incidents?filter={"order":"startTime DESC","where":{"unitsId":"'+ unitId +'","incidentType":{"inq":["Operations","operations","v2","V2"]},"or":[{"and":[{"open":false},{"endTime":{"gte":"'+endTime[0:10]+"T23:59:59.000Z"+'"}},{"startTime":{"lte":"'+ endTime[0:10]+"T00:00:00.000Z" +'"}}]},{"and":[{"open":true},{"startTime":{"lte":"'+endTime[0:10]+"T00:00:00.000Z"+'"}}]}]}}'
...
#sched.add_job(populateStreamData,'interval',minutes=5)
